...
struct task {
	struct promise_type {
		...
		task get_return_object() {
			std::cout << "2.create coroutine return object, and the coroutine "
			             "is created now\n";
			return {
		...
		std::suspend_never initial_suspend() {
			...
			std::cout << "4.don't suspend because return std::suspend_never, "
			             "so continue to execute coroutine body\n";
			return {};
		}

		std::suspend_never final_suspend() noexcept {
			std::cout << "13.coroutine body finished, do you want to susupend "
			             "the current coroutine?\n";
			std::cout
			    << "14.don't suspend because return std::suspend_never, and "
			       "the continue will be automatically destroyed, bye\n";
			return {};
		}

		void return_void() {
			std::cout << "12.coroutine don't return value, so return_void is "
			             "called\n";
		}
