...
namespace co_async {
    ...
    struct Ipv6Address {
        ...
        explicit Ipv6Address(const char* ip, int port) {
    ...
    struct AsyncSocket : AsyncFile {
        AsyncSocket() : AsyncFile(checkError(socket(AddrType::family, sockType, 0))) {}
    };
    ...
    inline AddrType socketGetAddress(AsyncFile& sock) {
        ...
        checkError(getsockname(sock.fileNo(), (struct sockaddr*)&addr.mSockAddr, &addrLen));
        return addr;
    }
    ...
    inline Task<void> socket_connect(EpollLoop& loop, AsyncFile& sock, auto const& addr) {
        sock.setNonblock();
        checkErrorNonBlock(connect(sock.fileNo(), (struct sockaddr const*)&addr.mSockAddr, sizeof(addr.mSockAddr)), 0, EINPROGRESS);
        co_await wait_file_event(loop, sock, EPOLLOUT);
        int err = socketGetOption<int>(sock, SOL_SOCKET, SO_ERROR);
        if (err != 0) [[unlikely]] {
            throw std::system_error(err, std::system_category(), "connet");
        }
    ...
    inline Generator<std::size_t> socket_send(EpollLoop& loop, AsyncFile& sock, std::span<char const> buffer) {
        std::size_t sent = 0;
        while (true) {
            co_await wait_file_event(loop, sock, EPOLLOUT);
            while (true) {
    ...
    inline Task<std::size_t> socket_recv(EpollLoop& loop, AsyncFile& sock, std::span<char> buffer) {
    ...
    inline Task<void> socket_bind(EpollLoop& loop, AsyncFile& sock, auto const& addr, int backlog = SOMAXCONN) {
        sock.setNonblock();
        checkError(bind(sock.fileNo(), (struct sockaddr const*)&addr.mSockAddr, sizeof(addr.mSockAddr)));
        co_await wait_file_event(loop, sock, EPOLLOUT);
        int err = socketGetOption<int>(sock, SOL_SOCKET, SO_ERROR);
        if (err != 0) [[unlikely]] {
    ...
    inline Task<std::tuple<AsyncFile, AddrType>> socket_accept(EpollLoop& loop, AsyncFile& sock) {
        ...
        socklen_t addrLen = sizeof(addr.mSockAddr);
        co_await wait_file_event(loop, sock, EPOLLIN);
        int res = checkError(accept4(sock.fileNo(), (struct sockaddr *)&addr.mSockAddr,
